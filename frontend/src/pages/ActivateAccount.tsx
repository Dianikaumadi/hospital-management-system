    ...
    <div className="grid w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-xl md:grid-cols-2">
      ...
      <div className="p-8 md:p-12">
        ...
        {!loading && invitation && <form onSubmit={submit} data-testid="activate-account-form" className="mt-8">
          ...
          <label className="mt-6 block text-sm font-medium">Create password<div className="relative"><LockKeyhole size={17} className="absolute left-3 top-3.5 text-slate-400"/><input required minLength={8} type="password" data-testid="activate-password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 py-3 pl-10 pr-4 outline-none focus:border-primary"/></div></label>
          <label className="mt-5 block text-sm font-medium">Confirm password<div className="relative"><LockKeyhole size={17} className="absolute left-3 top-3.5 text-slate-400"/><input required minLength={8} type="password" data-testid="activate-confirm-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 py-3 pl-10 pr-4 outline-none focus:border-primary"/></div></label>
          <button type="submit" data-testid="activate-submit" disabled={busy} className="mt-8 w-full rounded-xl bg-primary py-3 font-semibold text-white hover:bg-blue-700 disabled:opacity-60">{busy ? 'Activating...' : 'Create password and sign in'}</button>
        </form>}
